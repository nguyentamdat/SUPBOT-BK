...
// });
var rasa = require("../helper/rasa")({
    uri: process.env.RASA_URL
});
...
module.exports = function(controller) {
    ...
    const onMessage = async(bot, message) => {
        ...
        // handle normal chat
        if (get_response(intent)) {
            let reply = get_response(intent);
            reply = user.getNewMsg(reply);
            return await bot.reply(message, reply);
        }
        ...
        let res = await request(options);
        let reply = res.text === "" ? "Bạn nói gì vậy? Bạn nhắc lại cho mình nghe đi!" : res.text;
        reply = user.getNewMsg(reply);
